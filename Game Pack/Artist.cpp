...
SDL_Texture* Artist::gScaleTest = NULL;
...
Uint32 Artist::getPixelFormat()
{
	return SDL_PIXELFORMAT_RGBA8888;
}

bool Artist::lockTexture(SDL_Texture* tex, void** pixels, int* pitch)
{
	bool success = true;

	//Texture is already locked
	//if (pixels != NULL) //MIGHT NEED MAKE DYNAMIC or just remove
	//{
...
bool Artist::unlockTexture(SDL_Texture* tex)
{
	bool success = true;

	//Texture is not locked
	//if (mPixels == NULL) //MIGHT NEED MAKE DYNAMIC
	//{
...
//Copy TexA into TexB to the x and y pos givin (texA must fit into texB)
bool Artist::transposeTexture(SDL_Texture* texA, SDL_Texture* texB, int x, int y)
{
	//get width and height
	int aW, aH, bW, bH;
	SDL_QueryTexture(texA, NULL, NULL, &aW, &aH);
	SDL_QueryTexture(texB, NULL, NULL, &bW, &bH);
	//create places to hold pixels and pitch
	void* aMPixels, * bMPixels;
	int aPitch, bPitch;
	//lock both textures
	SDL_LockTexture(texA, NULL, &aMPixels, &aPitch);
	SDL_LockTexture(texB, NULL, &bMPixels, &bPitch);
	//Get usable pixel data
	Uint32* aPixels = (Uint32*)aMPixels;
	Uint32* bPixels = (Uint32*)bMPixels;
	int aPixelCount = (aPitch / 4) * aH;
	int bPixelCount = (bPitch / 4) * bH;
	//loop thru all the pixels in tex A
	for (int i = 0; i < aPixelCount; i++)
	{
		//determine where on bTex we need to copy too
		//get texA pixel column and row
		int row = int(i / aW);
		int column = i % aW;
		//set bPixel to aPixel
		bPixels[(row + y) * bW + column + x] = aPixels[i];
	}
	//unlock texs
	SDL_UnlockTexture(texA);
	SDL_UnlockTexture(texB);

	return true;
}

SDL_Texture* Artist::resizeTexture(SDL_Texture* tex, int w, int h)
{
	//create a new streamable texture with the sizes passed
	SDL_Texture* returnTex = SDL_CreateTexture(gRenderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, w, h);
	//Set texture blend mode 
	SDL_SetTextureBlendMode(returnTex, SDL_BLENDMODE_BLEND);
	//get a copy of the width and height of the old texture
	int oldW, oldH, newW, newH;
	SDL_QueryTexture(tex, NULL, NULL, &oldW, &oldH);
	SDL_QueryTexture(returnTex, NULL, NULL, &newW, &newH);
	//get a copy of both textures pixels and pitch
	void* newMPixels, *oldMPixels;
	int newPitch, oldPitch;
	//lock both textures
	SDL_LockTexture(returnTex, NULL, &newMPixels, &newPitch);
	SDL_LockTexture(tex, NULL, &oldMPixels, &oldPitch);
	//memcpy the old textures pixels onto the new one
	//make sure we are scaling it to the new size (if its 10 times bigger copy each pixel 10x10, if its half the size skip every other and so on)
	//get scale for BOTH dimensions
	float xScale, yScale;
	xScale = float(newW) / float(oldW);
	yScale = float(newH) / float(oldH);
	//Allocate format from window
	Uint32 format = SDL_PIXELFORMAT_RGBA8888;
	SDL_PixelFormat* mappingFormat = SDL_AllocFormat(format);
	//Get pixel data
	Uint32* oldPixels = (Uint32*)oldMPixels;
	Uint32* newPixels = (Uint32*)newMPixels;
	int oldPixelCount = (oldPitch / 4) * oldH;
	int newPixelCount = (newPitch / 4) * newH;

	//loop thru each pixel of new tex and determine what pixel it sould pull from the old one
	for (int i = 0; i < newPixelCount; i++)
	{
		//determine what oldPixel to copy
		int column = i % newW / xScale;
		//determine what row of the textures we are on
		//int row = i / newW;
		//add a whole row to the offset for each row we on of the old texture
		int row = int(i / newW) * newW;
		row = row / newW;
		row = int(row / yScale) * oldW;
		//copy the selected pixel from old to new
		newPixels[i] = oldPixels[column + row];
	}
	//maybe dont need mem copy
	//memcpy(newPixels, oldPixels, oldPitch);
	//unlock both textures
	SDL_UnlockTexture(returnTex);
	SDL_UnlockTexture(tex);
...
void Artist::setRenderTarget(SDL_Texture* tex)
{
	SDL_SetRenderTarget(gRenderer, tex);
}

SDL_Texture* Artist::loadTargetTexture(int w, int h)
{
	SDL_Texture* newTexture = NULL;
	//create blank targetable texture
	newTexture = SDL_CreateTexture(gRenderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, w, h);

	return newTexture;
}

SDL_Texture* Artist::loadStreamableTexture(std::string path)
{
	//Get rid of preexisting texture   dont need if we are just adding a new one
	//free();
...
std::vector<SDL_Texture*> Artist::loadAnimationData(std::string textureFolderLocation, std::string textureSetName, int textureSetNumber, int textureAmount)//NEEDS REMADE
{
	Artist artist;
	...
	{
		tempTexSet.push_back(artist.loadTexture("Resource/" + textureFolderLocation + "/" + textureSetName + "/" + std::to_string(textureSetNumber) + "/" + textureSetName + std::to_string(i) + ".png"));
	}
	return tempTexSet;
}
