...
public:
	...
	void drawAnimation(std::vector<SDL_Texture*> texSet, int x, int y, int frameTime, int animationStartTime, int width, int height, float r, SDL_RendererFlip flip);
	...
	bool transposeTexture(SDL_Texture* texA, SDL_Texture* texB, int x, int y);

	SDL_Texture* resizeTexture(SDL_Texture* tex, int w, int h);

	void setRenderTarget(SDL_Texture* tex);

	bool lockTexture(SDL_Texture* tex, void** pixels, int* pitch);

	bool unlockTexture(SDL_Texture* tex);

	SDL_Texture* loadTargetTexture(int w, int h);

	SDL_Texture* loadStreamableTexture(std::string path);
	...
	std::vector<SDL_Texture*> loadAnimationData(std::string textureFolderLocation, std::string textureSetName, int textureSetNumber, int textureAmount);
